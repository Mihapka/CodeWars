...
public class FindFirstNonConsecutiveNumber {
    ...
    (This is an empty array and one with a single number and is not tested for, but you can write your
    own example test. )
    ...
    public static void main(String[] args) {
        System.out.println("6:    " + find(new int[]{1, 2, 3, 4, 6, 7, 8}));
        System.out.println("null: " + find(new int[]{1, 2, 3, 4, 5, 6, 7, 8}));
        System.out.println("-1    " + find(new int[]{-5, -4, -3, -1}));
    }

    static Integer find(final int[] array) {
        return IntStream.range(1, array.length)
                .filter(i -> array[i] - array[i - 1] != 1)
                /* когда мапим JVM сама подбирает нужный тип -> Stream<Integer>*/
                .mapToObj(x -> array[x])
                .findFirst()
                .orElse(null);
    }
